...
func TestForkPoints(t *testing.T) {
	...
	*/
	...
	for _, tc := range tests {
		commitHash := nameToHash[tc.Commit]

		wantName := tc.WantForkPoint
		wantHash := nameToHash[wantName]

		gotHash, err := gw.FindForkPoint(tc.ForkedFrom, commitHash)
		assert.NoError(t, err, "FindForkPoint")
		gotName := hashToName[gotHash]

		assert.EqualStrings(t, wantHash, gotHash,
			"fork point for %v (%v), want=%v (%v) got=%v (%v)",
			tc.Commit, commitHash, wantName, wantHash, gotName, gotHash)

		gotIsMainCommit, err := gw.IsFirstParentAncestor(commitHash, "main")
		assert.NoError(t, err, "IsFirstParentAncestor")

		if tc.WantIsMainCommit {
			assert.IsTrue(t, gotIsMainCommit,
				"%v (%v) is main commit", tc.Commit, commitHash)
		} else {
			assert.IsTrue(t, !gotIsMainCommit,
				"%v (%v) is not main commit", tc.Commit, commitHash)
		}
